...
const PRODUCTS = [
  {
    ...
    category: "3-Step Kit",
    description:
      "Instant glow & shine with our 3-step kit: Peeling Cream (20 Vol), Brighter Powder & Soothing Lotion. Tri Lightening Complex with Vitamin C, E & Glutathione. Free serum inside.",
    price: 999,
    badge: "Bestseller",
    icon: "polish",
    image: "assets/products/skin-polish/kit.png",
    images: [
      "assets/products/skin-polish/kit.png",
      "assets/products/skin-polish/box-front.png",
      "assets/products/skin-polish/box-side.png",
      "assets/products/skin-polish/promo.png",
    ],
  },
  {
    id: "whitening-serum",
...
function renderProductMedia(product) {
  if (product.images?.length) {
    return `
      <div class="product-card__gallery" tabindex="0" aria-label="${product.name} photos — swipe to browse">
        ${product.images
          .map(
            (src, i) =>
              `<img src="${src}" alt="${product.name} — photo ${i + 1}" class="product-card__photo" loading="${i === 0 ? "eager" : "lazy"}">`
          )
          .join("")}
      </div>
    `;
  ...
  if (product.image) {
    return `<img src="${product.image}" alt="${product.name}" class="product-card__photo" loading="lazy">`;
  }
  return PRODUCT_ICONS[product.icon] || "";
}

function renderCartThumb(product) {
  if (product.image) {
    return `<img src="${product.image}" alt="" class="cart-item__photo">`;
  }
  return PRODUCT_ICONS[product.icon] || "";
}
...
function renderProducts() {
  ...
  grid.innerHTML = PRODUCTS.map(
    ...
    <article class="product-card" data-id="${p.id}">
      <div class="product-card__image${p.images?.length ? " product-card__image--gallery" : ""}">
...
function updateCartUI() {
  ...
  cart.forEach((item) => {
    ...
    div.innerHTML = `
      <div class="cart-item__thumb">${renderCartThumb(product)}</div>
